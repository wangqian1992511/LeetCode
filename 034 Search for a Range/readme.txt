...
The tricky point for me to get an O(log(n)) time complexity, instead of O(nlog(n)) (this is the worst case for the O(klog(n)), yes~), is the instruction "if ((nums[st] == target) && (nums[en] == target))".

For example, if we'd like to search the range for 2 in the array {1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3}.
